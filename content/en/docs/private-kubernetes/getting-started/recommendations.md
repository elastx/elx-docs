...
> **Note:** You do not need to follow this documentation on order to use your cluster

## Ingress and cert-manager

To make it easier to expose a lot of applications an ingress controller is a common to deploy.
...
Below we describe requests and limits briefly. However for a more detailed description or help setting requests and limits we recommend to check out Kubernetes documentation: https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/
...
If you are using auto scaling requests also help the scheduler when to add new nodes.
...
`CPU` limit you application will be throttled or simply run slower when trying to exceed the limit.
`Memory` limit are another beast. If any pod trying to use more memory than the limit the pod will be Out of memory killed.
...
We have built in support for the scale nodes up and down. To get started with auto scaling we recommend that you check out our [guide that can be found here](../../guides/autoscaling/)
...
Network policy can be used to limit both incoming and outgoing traffic. This allows you to for example specify that only a set of your pods are allowed to talk to the database.
