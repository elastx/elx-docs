...
This guide will help you getting started with Volume retyping in OpenStack's Horizon and CLI.<br/>
...
The volume status will change to retyping, this can take a long time depending on size of volume.
...
Retype volume with: ```openstack volume set --type 16k-IOPS --retype-policy on-demand <VolumeID/Name>```
...
When retyping is done, status will show as: ```available```.
